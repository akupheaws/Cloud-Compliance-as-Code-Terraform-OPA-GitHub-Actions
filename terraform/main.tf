...
########################################
# Optional: a simple SG so instances are reachable (tighten for prod)
########################################
resource "aws_security_group" "vm_sg" {
  name        = "opa-demo-sg"
  description = "Minimal SG for demo"
  vpc_id      = null # uses default VPC; set if you manage VPCs
  ...
  ingress {
    ...
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"] # tighten this in real envs
  }
...
# You can change env or type per instance for realism
########################################
locals {
  instances = {
...
resource "aws_instance" "vm" {
  ...
  # Merge global defaults with per-instance tags
  tags = merge(
    var.default_tags,
    {
